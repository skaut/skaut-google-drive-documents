...
function add_block() {
	...
	\Sgdd\enqueue_script( 'sgdd_inspector_js', '/public/js/inspector.js', [ 'wp-element', 'sgdd_integer_settings_js' ] );
...
function display( $attr ) {
	...
	if ( isset( $attg['embedWidth'] ) ) {
		$size .= 'width:' . $attr['embedWidth'] . 'px;';
	}

	if ( isset( $attr['embedHeight'] ) ) {
		$size .= 'height:' . $attr['embedWidth'] . 'px;';
	}

	$test   = print_file( $attr['fileId'] );
	
	if ( !isset( $test['id'] ) ) {
		$result = 'Niekde nastala chyba!' . $test;
	} else if ( isset( $attr['folder'] ) && 'true' === $attr['folder'] ) {
		$link   = $test['id'];
