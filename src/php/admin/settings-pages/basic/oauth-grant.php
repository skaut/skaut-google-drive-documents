...
function display() {
	...
	add_settings_error( 'general', 'help', sprintf( esc_html__( 'See the %1$sdocumentation%2$s for more information about how to configure the plugin.', 'skaut-google-drive-documents' ), '<a href="' . esc_url( 'https://github.com/skaut/skaut-google-drive-documents/wiki/N%C3%A1vod-na-nastavenie' ) . '" target="_blank">', '</a>' ), 'notice-info' );
	// translators: 1: Start of a link to the Google Console 2: End of the link to the Google Console.
	add_settings_error( 'general', 'help', sprintf( esc_html__( 'To get access details please follow this %1$slink%2$s', 'skaut-google-drive-documents' ), '<a href="' . esc_url( 'https://console.cloud.google.com/apis/dashboard' ) . '" target="_blank">', '</a>' ), 'notice-info' );

	settings_errors();
	echo '<p>' . esc_html__( 'Create a Google app and provide the following details:', 'skaut-google-drive-documents' ) . '</p>';
	echo '<a class="button button-primary" href="' . esc_url_raw( wp_nonce_url( admin_url( 'admin.php?page=sgdd_basic&action=oauth_grant' ) ) ) . '">' . esc_html__( 'Grant Permission', 'skaut-google-drive-documents' ) . '</a>';
}
